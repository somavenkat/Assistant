import { loadCachedMissions, mergeMissionLists, saveCachedMissions, upsertCachedMission } from './missionCache';
...
export async function createMission(payload: {
  ...
  const mission = await postApi('/missions', {
    request: payload.request,
...
export async function listMissions(includePreviews = false): Promise<MissionRecord[]> {
  ...
  const merged = mergeMissionLists(server, loadCachedMissions());
  saveCachedMissions(merged);
  return merged;
}
...
): Promise<ChatResult & { history?: ChatMessage[] }> {
...
export async function saveChatHistoryRemote(phone: string, history: ChatMessage[]): Promise<void> {
  if (!phone) return;
